...
rankeR <- function(dir, path,
                   permute_var = F, #a variable to permute for our simulations
                   permute_themes = F, #a permutation that randomizes theme groupings
                   weights=.5, #standard weighting scheme
                   nranks=10, 
                   clean_names=F,
                   debug=F) {
  ...
  theme_dt <- extract_theme %>% 
    setnames(., names(.), names(.) %>% tolower) %>% 
    .[, theme_weights := 1] %>% 
    .[themename=="Environmental Effects", theme_weights := weights] %>% 
  ...
  #randomize our theme map to generate a random weight permutation
  if(permute_themes!=F) {
    message('permuting by randomizing theme weights: ', permute_themes)
    random_theme_map <- measure_dt[, .(item, theme)] %>% 
      unique %>% 
      .[, theme := lapply(.SD, sample), .SDcols = 'theme']
    
    measure_dt <- merge(measure_dt[, theme:=NULL],
                        random_theme_map,
                        by='item')
  }
  ...
  theme_ranks <- measure_dt[, CJ(GEOID, item, unique=T), by=theme] %>% 
    ...
    #TODO changed to all.x for janky theme permutation, code better and change back
    merge(theme_dt, by=c('GEOID', 'theme'), all.x=T) %>% 
  ...
  index_ranks <- setorder(theme_ranks, GEOID, theme) %>% 
    ...
    .[, permutation := ifelse(permute_var!=F, permute_var, permute_themes)] #document the permute for this round
  ...
  if(permute_var %>% is.character | permute_themes) return(index_ranks)
