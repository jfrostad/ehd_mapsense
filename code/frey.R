...
ranks_per <- lapply(unique(dt$item)[-1] %>% as.character,
    ...
    #add on the rest of the variables for analysis
    merge(dt[level==1],
          by='GEOID',
          allow.cartesian=T)  %>% 
    #also merge on the theme names for coloring in plots
    .[, `:=`(item = NULL, theme = NULL)] %>% 
    merge(dt[level==3, .(item, theme)] %>% unique(by='item'),
          by.x='permutation',
          by.y = 'item')  
  
#also try a permutation that randomly assigns the themes (and therefore weights, 100x)
ranks_per <- lapply(1:100,
                    rankeR,
                    dir=data.dir, 
                    path=data_extract_EHDv2,
                    weights=.5,
...
#calculate uncertainty/accuracy stats for both:
#overall ranks
...
#do some plotting/mapping of the results
cartographeR(dt=rank_uncertainty, map_varname = 'range', map_label = 'Range in Ranking',
             map_title = 'Rank Range', scale_type='cont')
cartographeR(dt=rank_uncertainty, map_varname = 'rank_acc', map_label = 'Stability',
             map_title = 'Stability of Ranking', scale_type='cont')
cartographeR(dt=rank_uncertainty, map_varname = 'impacted_acc', map_label = 'Stability',
             map_title = 'Stability of Impacted Classification', scale_type='cont')

ggplot(rank_uncertainty, aes(x=rank, range, color=impacted_hierarchy %>% as.factor) ) +
  geom_point(position='jitter') +
  scale_color_brewer('Impact Status', palette = 'Paired') +
  scale_x_continuous('EHD Ranking') +
  scale_y_continuous('Permuted Range') +
  theme_bw() 

file.path(viz.dir, 'rank_range_scatters.png') %>% ggsave(height=8, width=12)

ggplot(rank_uncertainty, aes(x=rank, rank_acc, color=impacted_hierarchy %>% as.factor) ) +
  geom_point(position='jitter') +
  scale_color_brewer('Impact Status', palette = 'Paired') +
  scale_x_continuous('EHD Ranking') +
  scale_y_continuous('Rank Stability') +
  theme_bw() 
...
file.path(viz.dir, 'rank_acc_scatters.png') %>% ggsave(height=8, width=12)
  ...
  scale_x_discrete('Permuted Indicator') +
  scale_y_continuous('Maximum Deviation') +
  coord_flip()
