...
} else file.path(vizdata.dir, 'prepped_data.RDS') %>% readRDS %>% list2env(., globalenv())
...
# ---COINr--------------------------------------------------------------------------------------------------------------
##
#this module is built on the R package COINr, which is heavily documented below:
#https://bluefoxr.github.io/COINr/
##

#firstsetup COINr to run a sensitivity analysis across your param samples
...
potential_indicators_list <- c("asthma", 'wildfire_smoke', 'pesticides', 'lending_discrim')
...
#lending discrim
#TODO remove for training version
redline_dt <- file.path(data.dir, 'indicators', 'lending_discrim.csv') %>% 
  fread %>% 
  .[, .(uCode=GEOID10 %>% as.character, lending_discrim=`rank_hcl`)]
...
ehd_coin_dt <- ehd_coin_dt %>% 
  ...
  merge(redline_dt, by='uCode') #TODO remove for training version

#new metadata
ehd_coin_meta_new <- data.table(
  iCode=c('pesticides', 'asthma', 'wildfire_smoke', 
          'lending_discrim'), #TODO remove for training version
  iName=c('Pesticide Use', 'Asthma Rates', 'Wildfire Smoke Score',
          'Lending Discrimination'), #TODO remove for training version
  Direction=c(1,1,1,
              1), #TODO remove for training version
  Weight=c(1,1,1,
           1), #TODO remove for training version
...
#note that this method is by percentiles instead of deciles
...
coin_old <- change_ind(coin, drop = potential_indicators_list, regen = TRUE)
...
coin_lend <- change_ind(coin_old, add = 'lending_discrim', regen=T) #TODO remove for training version
...
coin_list <- list(
  'coins'=list(coin_old, coin_asthma, coin_smoke, coin_pest, coin_poc,
               coin_lend), #TODO remove for training version
  'names'=list('ehd_rank', 'ehd_asthma', 'ehd_smoke', 'ehd_pest', 'ehd_white',
               'ehd_lend') #TODO remove for training version
)
...
#TODO remove for training version
cartographeR(dt=sens_dt, map_varname = 'lend_diff', map_label = 'Difference w/ \n% Mortgage Discrim',
             map_title = '',
             tag = 'coin_results', scale_type='div_man', scale_vals = div_colors,
             get_plot=T)
...
testChange(dt=sens_dt, this_col='ehd_lend') #TODO remove for training version
